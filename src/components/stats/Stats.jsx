...
const REACT_APP_USERNAME = process.env.REACT_APP_USERNAME
const GITHUB_USER = process.env.REACT_APP_GITHUB_USER
const REACT_APP_GITHUB_PAT = process.env.GITHUB_PAT
...
const fetchGitHubProfile = async () => {
  ...
  let { data } = await octokit.request(
    `GET /users/${GITHUB_USER}/repos?per_page=300`
  );
  let repos = data;
  ...
  for (let repo of repos) {
    ...
    let res = await octokit.request(
      `GET /repos/${GITHUB_USER}/${repo.name}/pulls?state=all`
    );
    result.totalPRs += res.data.length;
    console.log(result.totalPRs);

    const { data } = await octokit.request(
      `GET /repos/${GITHUB_USER}/${repo.name}/commits?per_page=300`
    );
    for (let comm of data) {
      if (comm?.author?.login === `${GITHUB_USER}`) {
  ...
  result.totalPRs +=
    (
      await octokit.request(
        "GET /repos/arjunarj/dE-Vote/pulls?state=all"
      )
    ).data.length +
    (
      await octokit.request(
